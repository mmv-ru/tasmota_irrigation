...
- **Планировщик**: `auto_flood()` по cron (часы 14–01) — если `IsDry()` и не идёт сессия → блок 1: сохраняет Prev*-статистику + estimate-входы одной партией (`_persist_batch`); блок 2 (init): `LastFloodVol=0`, `AutofloodInProcess=true`, `SoilHPreFlood=RawEma`, сброс `Max/Confirmed/Time`; блок 3: `start_flood()`.
