...
6. **PersistStore (проект irrigation)**: весь persist в `watering.be` идёт через класс `PersistStore` (`self.Store`, создаётся в `Watering.init()`, `load()` перед `init_sensors`). Реестр из 11 ключей (`register(name, meta{default, policy, thr})`): `TargetDry`(800), `TargetWet`(760), `LastFloodVol`(0) + 8 ключей сессии (`SoilHPreFlood`, `SoilHPostFlood`, `SoilMaxHymidity`, `SoilMaxHymidityTime`, `PrevSoilMaxHymidity`, `PrevSoilHPreFlood`, `PrevFloodedVol`, `PrevSoilHPostFlood`). `Cache[name]` — актуальное значение, `Shadow[name]` — последнее записанное на Flash. **Запись отложенная**: `set()` сразу пишет значение в persist-мапу (в память), но `persist.save()` выполняется по политике: `immediate` (сразу), `debounced` (дефолт, 15s таймер `ID_PERSIST_SAVE` с пере-армом), `threshold` (относительный порог: `|value-shadow|/base >= thr`, `base = shadow`, при `shadow==0` → `base=value`, оба 0 → false). `flush(force)` пишет все ключи одним save; `save_batch(names, source)` — батч (один save). Калибровка Dry/Wet (`SoilSensor.setmember('Dry'/'Wet')`, `SetDry/SetWet`) пишет `TargetDry`/`TargetWet` через `Store.set` (debounced); если `Store == nil` (standalone) — прямой `introspect.set` + `save()`. В `init_sensors()` ключи читаются через `self.Store.get(...)`, как **RAW-значения**; setter принимает **% влажности** (конвертация через `Hymidity2Raw()`).
7. **`auto_flood` / `every_second` (проект irrigation)**: prev-статы (`PrevSoil*`, `PrevFloodedVol`) и estimate-входы (`SoilHPreFlood`/`LastFloodVol`/`SoilMaxHymidity`) пишутся ОДНОЙ партией через `self.Store.save_batch([...], self)` — цикл `introspect.set` + ОДИН `persist.save()` (7 ключей за один save). `every_second` (подтверждение SoilMaxHymidity) — `save_batch(['SoilMaxHymidity','SoilMaxHymidityTime'])`. Метода `_persist_batch` больше нет. `deinit()` — `Store.flush(true)` (форс сброса недописанного) + снятие таймера. Дамп: команда `Store` (JSON всех ключей) + **веб-таблица только в detail-режиме** под аккордеоном SoilA1Hymidity (см. ниже).
...
3. **Detail-блок в аккордеоне SoilA1Hymidity**: в detail-режиме после soil1 эмитятся `{s}|`-ряды: `SoilHymidity1 max`(+`max time`) и снапшот persist-таблицы `Store.*` в явном порядке «текущая сессия → предыдущая» (`LastFloodVol`, `SoilHPostFlood`, `SoilHPreFlood`, `PrevFloodedVol`, `PrevSoilHPostFlood`, `PrevSoilHPreFlood`, `PrevSoilMaxHymidity`), значения `nil` выводятся как `nil`. **Исключены** 4 ключа-дубля рядов soil1: `TargetDry`, `TargetWet`, `SoilMaxHymidity`, `SoilMaxHymidityTime`. В компактном режиме max- и Store-ряды не выводятся вовсе. «Last flood time» и flow — отдельные секции, не трогаются.
...
- **Планировщик**: `auto_flood()` по cron (часы 14–01) — если `IsDry()` и не идёт сессия → блок 1: сохраняет Prev*-статистику + estimate-входы одной партией (`Store.save_batch`, один save); блок 2 (init): `LastFloodVol=0`, `AutofloodInProcess=true`, `SoilHPreFlood=RawEma`, сброс `Max/Confirmed/Time`; блок 3: `start_flood()`.
- **`start_flood()`** — единая точка запуска полива (используется `auto_flood`, повтором и кнопкой). Дry-check: если `RawEma <= RawWet` → лог `"soil not dry (RawEma<=RawWet), skip"` и возврат (помпа НЕ включается); иначе `PlannedFlood = planned_dose()` (= `estimateflood()` при валидной оценке, иначе `Counter1FloodDefault`) и `Power1 1`. Кнопка и повтор НЕ делают init-сессии (`AutofloodInProcess` выставляется только `auto_flood`).
- **`estimateflood()`** строит оценку на **Последней завершённой сессии — Prev*** (`PrevSoilHPreFlood − PrevSoilMaxHymidity`, `PrevFloodedVol`), которые копируются в блоке 1 ДО обнуления текущих полей и персистятся там же — поэтому оценка работает и после ребута. До рефакторинга читал текущие поля до их обнуления.
- **Старт** (`water_on()`, вызывается диспетчером `rule_power`): защита от запуска на мокрой почве (`IsWet()` → `Power1 0`), TelePeriod 10, `FinishRule = COUNTER#C1 >= before+backflow+доза`, `RateMeasuring=true`.
- **Лимит** (`rule_flooded`): превышен счётчик → `Power1 0`.
- **Стоп** (`water_off()`): читает Counter1 из сенсоров, хелпер `_compensate_backflow(Counter1)` — вычитает backflow из счётчика (preset `counter1`) и из дельты, возвращает нетто; `_record_flood(CounterDelta)` — `LastFloodVol += delta`, задержка проверки 2ч (24ч при `LastFloodVol > MaxFlood/2`) + таймер `timer_soil_transition_after_flooded`; при `CounterDelta==0` вместо него `_end_session_no_water()` (закрывает сессию без проверки почвы). Затем таймер возврата TelePeriod 300 через 60с.
- **Проверка результата** (`timer_soil_transition_after_flooded`, через 2ч/24ч): земля всё ещё сухая → `Counter1FloodDefault × 1.2` (cap `MaxFlood`) и повторный полив через `start_flood()` (доза пересчитывается заново; при валидном `estimateflood` эскалация default×1.2 в текущий заход не применится — применяется только при fallback);
...
- **Фон** (`every_second`, 1/с): `read_sensors` → Update сенсоров; трекинг минимума `SoilMaxHymidity` (кроме периода `PauseSoilMaxStat`), подтверждение после роста +5 — через `Store.save_batch(['SoilMaxHymidity','SoilMaxHymidityTime'])` (один save).
