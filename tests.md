...
| `19_web_sensor` | Веб-строки (сырой HTML-секции): detail soil1: группы `Уставки/Датчик/Сеанс/Размачивание/Текущий сеанс/Предыдущий сеанс`, стек `Сухо/Влажно` (`raw` + `<span class='stk'>% u`), max-ряд, Dry soak/Dry threshold при `me=1`, `me=` — нет; строки detail без префикса `|`; Common-секция: `me=c` даёт flow-детали (pulse/s, ml/min) + Calibration mode; пер-канал: `me=2` → свои max/max time и префикс `Store.P2*` (P1* и soil1 max скрыты); `Сеанс полива</th>` (`active`|`wait`) и `Вода</th>` (`run`|`idle`) — только в detail развёрнутого канала: в компакте (`me=`) рядов НЕТ, у активного канала `active` ровно один раз, у idle-канала `Вода=idle`; статус-иконка в заголовке: тултип-легенда всех 3 статусов с маркером текущего (`→ 💧 Ожидание` по одному на канал в компакте, активный канал переключает на `→ ⏳ Сеанс полива`, при `PowerN=1` — `→ 💦 Работа насоса` + `Вода=run` с живым `ml/min`); заголовки по `NumChannels` (default 4): «Канал 1»..«Канал 4» есть, «Канал 5» нет |
...
| `29_rule_power_helpers` | Хелперы OFF-ветки `rule_power`: `_compensate_backflow` (все 3 ветки: дельта выше/ниже backflow), `_record_flood` (накопление объёма, 2ч таймер, пере-арм), `_end_session_no_water`; полный проход OFF через `rule_power` |
