...
| `23_web_deinit` | web_add_main_button (HTML: кнопка Calibration есть, мёртвая кнопка Toggle Conf УБРАНА, `Reset water counter` с главной УБРАН — перенесён в Common detail; инжект содержит `_wdSettingsOpen`, суффиксные `m_soildry_`/`m_ema_` и поле `wds_ema`; bare-форм Soil Dry/Wet/Dry threshold/Soak dose на главной НЕТ), deinit: снятие правил/cron/cmd, off насоса, `Store.flush(true)` (persist.save) |
