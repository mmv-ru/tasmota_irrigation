...
assert_eq(wp1.Power1, 1, "relay on via dispatcher")
assert_eq(wp1.FinishRule, "COUNTER#C1>=300", "finish rule set")
...
assert_eq(wp1.FinishRule, nil, "no finish rule armed on wet abort")
...
assert_eq(wp1.FinishRule, "COUNTER#C1>=300", "finish rule set")
assert_true(wp1.Counter1BeforeStart == 0, "start counter captured")
...
assert_true(real(wp1.LastFloodVol) == 250, "flood volume accumulated")
...
SIM['cmds'] = list()
wp1.water_off()
assert_eq(wp1.PauseSoilMaxStat, false, "pause lifted on empty session")
assert_eq(wp1.AutofloodInProcess, false, "session closed without water")
assert_true(SIM['timers'].find("ID_SOILTRANSITION_AFTERFLOOD") == nil, "no soil check timer for empty session")
