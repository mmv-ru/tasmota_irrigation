...
- Управление каналами, сервисные прогоны и калибровка датчика потока живут на `/svc` (рендерятся только при активном режиме):
  ...
  - Калибровка: `?cal=1&vol=мл` — `scale = объём / тики последнего прогона`; `?set=1&scale=коэф` — ручной ввод мл/тик. Результат (`service_set_scale`) пишется глобально (C1 общий) в Store-ключ `FlowScale` (default `0.1449`, политика `debounced`); `init_sensors()` читает его при старте (невалидное значение → фолбэк на встроенный scale).
