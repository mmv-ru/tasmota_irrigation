...
- Управление каналами, сервисные прогоны и калибровка датчика потока живут на `/svc`. В **выключенном** режиме страница тоже показывает текущий `Scale (мл/тик)` C1, форму прямого ввода `?set=1&scale=коэф` и — если калибровка уже была — снэпшот `LastCalibration` (канал, объём, тики, время, тиков/мин), т.е. смотреть scale/менять коэффициент можно без включения режима (`cal` по объёму и кнопки каналов доступны только при активном режиме). При активном режиме:
